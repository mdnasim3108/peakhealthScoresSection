...
const progressPoint = (props) => {
  const classes = `${props.progress.working || props.progress.completed ? "text-indigo-400" : "text-gray-400"
    }`;
  return (
    <div className={`flex flex-col lg:flex-row  w-full relative right-6 md:right-0`}>
      ...
      <div className="flex flex-row lg:flex-col lg:w-[3rem]  w-full items-center">
        ...
        {props.progress.working && <div className="rounded-full pulse shrink-0  lg:w-[20px] lg:h-[20px] w-[18px] h-[18px]  border-[3px] border-indigo-400" />}

        {props.progress.completed &&
          <div className="bg-indigo-400 lg:w-[20px] lg:h-[20px] w-[18px] h-[18px] shrink-0 rounded-full flex items-center  justify-center transition-all duration-300 ease-linear">
        ...
        {!props.progress.notextend && <div className="lg:h-[15vh] lg:w-[0.15rem] h-[0.18rem] w-full bg-gray-200 border-0 rounded">
          <div
            className={`w-full   transition-all duration-[600ms] ease-linear bg-indigo-400 ${props.progress.completed ? "lg:w-full h-full" : "lg:h-[0rem] w-[0rem]"
              }`}
          />
        </div>}
