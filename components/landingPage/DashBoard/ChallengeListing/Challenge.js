...
const challenge = (props) => {
    ...
    return <div className="mt-8 w-full">
        <ToastContainer />
        <div className="w-full lg:py-6 lg:pl-10 lg:pr-7 p-3  flex justify-between items-center bg-[#F8F9FAFF] transition-all duration-150 ease-in-out">

            <h1 className="lg:text-[24px] text-[20px] text-[#171A1FFF] font-[600]">{props.challenge.Challenge_Title ? props.challenge.Challenge_Title : ""}</h1>
            ...
            >
                {({ remainingTime }) => {
                    return (
                        ...
                        <div className=" border border-[#3F4FDBFF]  flex items-center justify-center w-[3rem] h-[2rem]">
        ...
        {challengeState.accept && <div className="lg:w-[700px] pl-10 pr-5 flex lg:flex-row flex-col justify-between mt-8 items-center">


            < div >

                <div className="border rounded shadow-lg sm:w-[160px] w-[120px] h-[64px] p-2 inline-block mr-2">

                    <h1 className="text-[20px] text-[#3F4FDBFF]">{props.challenge.
                        Challengers_Count ? props.challenge.Challengers_Count : ""
                    }</h1>

                    <p className="text-[#9095A1FF] text-[12px]">challengers</p>
                </div>

                <div className="border rounded shadow-lg sm:w-[160px] w-[120px] h-[64px] p-2 inline-block">

                    <h1 className="text-[20px]   text-[#3F4FDBFF]">{props.challenge.Challenge_Effort ? props.challenge.Challenge_Effort : ""}</h1>
        ...
        {challengeState.inst &&
            <div className="w-full mt-4">
                ...
                <ul className="w-full flex flex-col h-auto justify-between mt-3 gap-[2vh]">
                ...
                <div className="w-full text-center mt-8 cursor-pointer">
                    {!challengeState.count && <div className="inline-block mr-3" onClick={() => {
                        setChallengeState({ ...challengeState, inst: false, accept: true })
                        props.back()
                    }}>
            ...
            <div className="w-full mt-5">
                ...
                <div className="w-full text-center">
                    ...
                    <button className={`border border-[#3F4FDBFF] ${challengeState.status === "comp" && "bg-[#3F4FDBFF]"} py-1 px-4 border-r-2`}
                        onClick={!challengeState.status.length ? () => setChallengeState({ ...challengeState, status: "comp" }) : () => { }}
                    >
                    ...
                    <button className={`border border-[#3F4FDBFF] ${challengeState.status === "notComp" && "bg-[#3F4FDBFF]"} py-1 px-4 border-l-2`}
                        onClick={!challengeState.status.length ? () => setChallengeState({ ...challengeState, status: "notComp" }) : () => { }}
                    >
                ...
                {challengeState.status.length > 0 && <div className="w-full text-center cursor-pointer mt-5">
